	...
	"jquery-ui.min.js": function( callback ) {
		...
		this.jsBundle.promise.then(function( js ) {
			var _banner = banner( pkgJson, jsFileNames, { minify: true } ),
				uglifyJsOutput = UglifyJS.minify( js, { foo: "bar" } );
			console.log( "1 uglifyJsOutput", uglifyJsOutput );
			if (uglifyJsOutput.error) {
				callback( uglifyJsOutput.error );
				return;
			}
			callback( null, _banner + uglifyJsOutput.code );
		}).catch( callback );
	},
