	...
	min: function( file, options ) {
		...
		if ( !minified[ file.path ] || options.skipCache ) {
			...
			if ( (/\.js$/i).test( file.path ) ) {
				uglifyJsOutput = UglifyJS.minify( file.data.toString( "utf8" ), { baz: "qaz" } );
				console.log( "2 uglifyJsOutput", uglifyJsOutput );
				if (uglifyJsOutput.error) {
					logger.error( uglifyJsOutput.error );
				} else {
					minified[ file.path ].data = uglifyJsOutput.code;
				}
